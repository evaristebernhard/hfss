...
\title{9--11.5 GHz 四路魔 T 功率合成器参数说明}
\author{}
\date{\today}
...
\begin{abstract}
本文记录当前 HFSS 模型使用的初始参数。设计指标为：9--11.5 GHz 内回波损耗和端口隔离度均大于 22 dB，合成效率不低于 95\%，峰值功率不低于 30 kW，连续平均功率不低于 1 kW。下面的尺寸先由 WR90 模态范围、两节阻抗变换计算和双脊截面计算确定，再用于 HFSS 首轮建模。
\end{abstract}
...
\]
这个带宽不适合只用单节四分之一波变换或一个调谐螺栓来匹配。采用的结构顺序为
\[
\boxed{
\text{WR90 基准}
\rightarrow
...
\]
因此 9--11.5 GHz 位于 WR90 的 TE$_{10}$ 单模窗口内。直波导段的模态问题相对简单，重点放在魔 T 结区、双脊截面变化和螺栓附近的局部高阶模。
...
先在 reduced model 中假设
\[
Z_L=0.5Z_0.
\]
这里的 0.5 只是计算用的初始假设，不代表真实三维魔 T 结区就是一个严格的二分之一标量负载。
...
早期模型给出的两节长度都约为 9.601 mm。真实 ridge 会改变截止频率和传播常数，因此
\[
\beta_{\rm ridge}(f)\neq\beta_{\rm WR90}(f).
\]
两节的 ridge 尺寸和物理长度需要一起调整。
...
\]
范围内，reduced model 的最差回波都在 33.4--33.5 dB 左右。因此 ridge width 主要按加工条件、高阶模裕量和高功率 gap 来选，不再区分这零点几 dB 的差别。
...
因此后续建模采用这组 Cohn/TRM 参数；早期工程参数不再作为建模基线。
...
这个 33.5 dB 只是两节 ridge 匹配器的 reduced-model 结果，不能代替完整魔 T 的 HFSS 结果。
...
因此第二节 gap 暂时保持在约 5 mm，不继续压缩到 1--2 mm。
...
下一步是在真实三维魔 T 结区中检查模态、等效负载、局部峰值场和四路幅相一致性。
