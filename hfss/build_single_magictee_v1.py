#!/usr/bin/env python3
"""
HFSS 2022 / PyAEDT rough v1 builder.
...
--------
Build only, GUI:
    python build_single_magictee_v1.py

Build + solve:
    python build_single_magictee_v1.py --solve
...
def _assign_ports_and_walls(hfss, geom):
    ...
    # Z-propagating E arm: E is along Y.
    yline_4 = [
        [0.0, -B / 2.0, e_port_z],
        [0.0, B / 2.0, e_port_z],
    ]
...
def build(args):
    ...
    project_path = outdir / "single_magictee_v1.aedt"
    ...
    if args.solve:
        ...
        touchstone = outdir / "single_magictee_v1.s4p"
        try:
...
def parse_args():
    ...
    p.add_argument(
        "--output",
        default=str(Path(__file__).resolve().parent / "output_v1"),
