#!/usr/bin/env python3
"""
HFSS 2022 / PyAEDT H-junction candidate v3 builder.
...
This is NOT a production magic-tee geometry.  It is an H-only structural
candidate used to test whether a junction-local reduced-width throat supplies
the missing high-frequency H/even coupling direction.  The E arm remains the
validated v2 parity geometry.  The model is used to validate:
    - port orientation,
    - 9--11.5 GHz sweep,
...
--------
Build only, GUI:
    python build_single_magictee_v3_hcandidate.py

Build + solve:
    python build_single_magictee_v3_hcandidate.py --solve

Target 2022 R1 instead of 2022 R2:
    python build_single_magictee_v3_hcandidate.py --version 2022.1
"""
...
TERMINAL_RIDGE_LEN = 7.5       # Stage-A best empirical H starting point
H_THROAT_A = 18.80              # reduced broad dimension in junction-local H throat [mm]
H_THROAT_LEN = 5.50             # clean throat length before terminal ridge [mm]
H_TERMINAL_RIDGE_W = 4.572      # terminal ridge width, independent control [mm]
PORT_STRAIGHT_LEN = 10.0       # un-ridged WR90 section before the H-port
H_ARM_ACTIVE_LEN = PORT_STRAIGHT_LEN + TAPER_TOTAL + TERMINAL_RIDGE_LEN + H_THROAT_LEN
...
def _launch_hfss(project_path: str, version: str, non_graphical: bool):
    ...
    if api == "ansys.aedt.core":
        return Hfss(
            ...
            design="SingleMagicT_v3_HCandidate",
            version=version,
            solution_type="Modal",
    ...
    return Hfss(
        ...
        designname="SingleMagicT_v3_HCandidate",
...
def _subtract_h_junction_throat(hfss):
    """Create a clean, mirror-symmetric reduced-width H-plane throat.

    The throat begins directly at the H-plane tee mouth and ends before the
    terminal double-ridge section.  It is deliberately separated from the
    ridge taper so the new degree of freedom can be interpreted as a
    junction-coupling modifier rather than a hybrid iris/ridge cell.

    H arm propagates along +Y, so reducing the broad dimension means equal
    sidewall intrusions from x=+/-A/2.
    """
    if not (0.0 < H_THROAT_A <= A):
        raise ValueError("H_THROAT_A must satisfy 0 < H_THROAT_A <= A")
    if H_THROAT_LEN < 0.0:
        raise ValueError("H_THROAT_LEN must be non-negative")

    side_depth = 0.5 * (A - H_THROAT_A)
    if side_depth <= 1e-9 or H_THROAT_LEN <= 1e-9:
        return {
            "h_throat_start_y": A / 2.0,
            "h_throat_end_y": A / 2.0 + H_THROAT_LEN,
...
def build(args):
    global RIDGE_W, G1, L1, G2, L2, TERMINAL_RIDGE_LEN, H_THROAT_A, H_THROAT_LEN, H_TERMINAL_RIDGE_W, TAPER_TOTAL, H_ARM_ACTIVE_LEN
    ...
    TAPER_TOTAL = L1 + L2
    H_ARM_ACTIVE_LEN = (
        PORT_STRAIGHT_LEN + TAPER_TOTAL + TERMINAL_RIDGE_LEN + H_THROAT_LEN
    )

    outdir = Path(args.output).resolve()
    outdir.mkdir(parents=True, exist_ok=True)

    project_path = outdir / "single_magictee_v3_hcandidate.aedt"
    ...
    throat = _subtract_h_junction_throat(hfss)
    ...
    print("Design: SingleMagicT_v3_HCandidate")
    ...
    print("H-junction candidate:")
    print("  throat broad dimension = %.3f mm" % H_THROAT_A)
    print("  throat length = %.3f mm" % H_THROAT_LEN)
    print("  symmetric sidewall shoulder depth = %.3f mm" % ((A-H_THROAT_A)/2.0))
    print("  terminal ridge width = %.3f mm" % H_TERMINAL_RIDGE_W)
    ...
    if args.solve:
        ...
        touchstone = outdir / "single_magictee_v3_hcandidate.s4p"
        try:
...
def parse_args():
    ...
    p.add_argument(
        "--output",
        default=str(Path(__file__).resolve().parent / "output_v3_hcandidate"),
