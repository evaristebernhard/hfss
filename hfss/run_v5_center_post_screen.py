#!/usr/bin/env python3
"""Screen a strong rounded center post at the Magic-T junction."""
...
CASES = [
    Case("center_r1p5_h2p0", 1.5, 2.0),
    Case("center_r1p5_h3p5", 1.5, 3.5),
    Case("center_r1p5_h5p0", 1.5, 5.0),
    Case("center_r2p5_h3p5", 2.5, 3.5),
]
...
def main():
    ...
    manifest = {"program": "v5 junction-center post screen", "cases": []}
    ...
    for case in CASES:
        ...
        cmd = [
            ...
            "--h-throat2-len", "0",
            "--h-iris-open", "0",
            "--center-post-radius", str(case.radius),
            "--center-post-height", str(case.height),
            "--solve", "--non-graphical",
        ]
        manifest["cases"].append({"name": case.name, "radius_mm": case.radius, "height_mm": case.height, "output": str(target)})
